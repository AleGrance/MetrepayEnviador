...
@Component({
  selector: 'app-enviador-metrepay',
  templateUrl: './enviador-metrepay.component.html',
  styleUrls: ['./enviador-metrepay.component.css'],
})
export class EnviadorMetrepayComponent implements OnInit {
    ...
    if (files.length) {
      ...
      reader.onload = (event: any) => {
        ...
        if (sheets.length) {
          ...
          this.totalDestinatarios = this.clientesWa.length;
          ...
          for (let i = 0; i < this.clientesWa.length; i++) {
            if (i === this.index) {
              ...
              this.linkMetrepay = this.clientesWa[this.index].LINK_METREPAY;
            }
          }
        }
      };
  ...
  async enviarTodos() {
    ...
    for (let i = 0; i < this.clientesWa.length; i++) {
      ...
      this.nombreCliente = this.clientesWa[i].NOMBRE;
      this.objWa.message =
        this.mensajeSaludo + ' ' + this.nombreCliente + '. ' + this.mensajeWa + ' ' + this.linkMetrepay;
    ...
    this.api
      ...
      .pipe(
        map((result: any) => {
          ...
          if (result.responseExSave.unknow) {
            // Se muestra el card oculto que contiene los nros sin whatsapp
            (<HTMLInputElement>(
              document.getElementById('contenedorSinWhatsapp')
            )).style.display = 'block';

            // Se crea y se carga el elemento a insertar dentro del card con el nro devuelto por la API
            let noWhatsappDiv = (<HTMLInputElement>(
              document.getElementById('noWhatsapp')
            )).innerHTML;
            let numeroSinWha =
              `
                <p class="card-text" style="margin: 0px;">` +
              result.responseExSave.unknow +
              `</p>
              ` +
              noWhatsappDiv;
            (<HTMLInputElement>(
              document.getElementById('noWhatsapp')
            )).innerHTML = numeroSinWha;

            this.changeProgressBar(this.index + 1);
            // Llama al funcion que verifica si ya se recorrió toda la lista
            this.checkTotalEnvios();
          }
  ...
  // Verifica si ya se recorrió toda la lista. Es llamada en 2 ocaciones
  checkTotalEnvios() {
    if (this.index === this.clientesWa.length - 1) {
      setTimeout(() => {
        this.toastr.info(
          'Se completó el envío masivo! Total de envios: ' +
  ...
  /**
   * METODOS DE VERSIONES ANTERIORRES
   * DESCOMENTAR enviarTodos() y envioRetrasado()
   * COMENTAR async enviarTodos() y enviarMensaje(objWa: any)
